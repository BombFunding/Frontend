...
const PositionItem = ({ positionData, setPositions, id }) => {
  ...
  const deletePosition = () => {
    deleteData(`/position/delete/${id}/`)
      ...
      .catch((err) => {
        toast.error(
          <CustomToast Header="خطا" Message="مشکلی در حذف پوزیشن وجود دارد" />
        );
      });
  };
  return (
    <Card className={styles.card_style}>
      <div className={styles.col_box}>
        <Label className="text-base font-bold">{positionData?.name}</Label>
        ...
        <div className="self-start">
          ...
          <Label>
            {Math.round((end.getTime() - now.getTime()) / day_duration)} روز
          </Label>
        </div>
        <Progress
          value={
