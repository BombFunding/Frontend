      ...
      reader.onloadend = () => {
        ...
        postImageData("/auth/update_baseuser_profile/", formData)
          .then((res) => {
            ...
            setAvatar(`http://104.168.46.4:8000${res.profile.profile_picture}`);
            toast.dismiss(toastId);
            toast.success(
              <ErrorMessage message={"آواتار با موفقیت بارگزاری شد"} />
            );
          })
          .catch((err) => {
            ...
            setImageLoading(false);
            toast.dismiss(toastId);
            toast.error(<ErrorMessage message={"آواتار بارگزاری نشد"} />);
          });
      };
